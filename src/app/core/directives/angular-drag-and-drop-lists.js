 ...
 */
     ...
     * - dnd-auto-scroll      Optional boolean expression. When it evaluates to true, the list will
     *                        automatically scroll up and down (or left and right if
     *                        `dnd-horizontal-list` is set to true)
     *                        when an element is being dragged over. When the element approaches the
     *                        first quarter of the window it will scroll up (or left). When the
     *                        element approaches the last quarter of the window it will scroll down
     *                        (or right). Same mechanics is done if the list has predefined height
     *                        and the `overflow` is set to `auto` or `scroll`.
     * - dnd-auto-scroll-speed Optional number. Sets the speed of the auto scrolling feature.
     *                         Defaults to 30.
     *
     * CSS classes:
     ...
     */
    .directive('dndList', ['$parse', '$timeout', '$interval',
      'dndDropEffectWorkaround', 'dndDragTypeWorkaround',
      function($parse,   $timeout,   $interval,
               dndDropEffectWorkaround,   dndDragTypeWorkaround) {
        return function(scope, element, attr) {
          ...
          var autoScrollSpeed = (attr.dndAutoScrollSpeed && scope.$eval(attr.dndAutoScrollSpeed)) || 30;
          ...
          element.on('dragover', function(event) {
            ...
            if (autoScroll) {
              ...
              // Only update the mouse pointer position so that `handleAutoScroll` can have an update
              // and properly check the boundaries. This should preferrably set in a mousemove event
              // handler but it's not triggered when dragging an element over.
              mousePointer = horizontal ? event.clientX : event.clientY;
            }
           ...
           * auto scroll the window using `window.scrollBy`. The mouse pointer is updated by the
          ...
          function handleAutoScroll() {
            ...
            if (mousePointer > nodeMaxEdge) {
              listNode.scrollTop += autoScrollSpeed;
